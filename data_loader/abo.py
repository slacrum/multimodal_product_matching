...
class ABO(Dataset):
    ...
    def _load_txts(self):
        if not (os.path.exists(os.path.join(self.path, "listings/listings.csv.gz")) or os.path.exists(os.path.join(self.path, "listings/listings.csv"))):
            ...
            json_pattern = os.path.join(self.path,'listings/metadata/listings_*.json.gz')
        ...
        if os.path.exists(os.path.join(self.path, "listings/listings.csv.gz")):
            ...
            dfs = pd.read_csv(os.path.join(self.path, "listings/listings.csv"), dtype=object)
        ...
        dfs = dfs.loc[(dfs['item_name.language_tag'] == "en_US") | (dfs['item_name.language_tag'] == "en_GB") | (dfs['item_name.language_tag'] == "en_IN")]
        dfs.reset_index(drop=True, inplace=True)

        dfs["product_type"] = dfs["product_type.value"]

        dfs = dfs[["item_keywords.value", "item_id", "item_name.value", "product_type", "main_image_id"]]
