...
class MNNEM(object):
    def __init__(self, img_input_size, txt_input_size, txt_weights, char_cnn_config, fc_layers, combined_fc_layers, learning_rate, metrics=["recall", "precision", "binary_accuracy", "cosine_similarity"], loss='binary_crossentropy') -> None:
    ...
    def _build_model(self):
        ...
        # fc + ReLU
        for i, fl in enumerate(self.fc_layers[:-1], 1):
            x = Dense(fl, activation='relu',
                      name=f"Image_FC_{i}")(img_features if i == 1 else x)

        # fc + L2 Norm
        x = Dense(
            self.fc_layers[-1], kernel_regularizer='l2', name="Image_FC_last")(x)
        ...
        # fc + L2 Norm
        x = Dense(
            self.fc_layers[-1], kernel_regularizer='l2', name="Text_FC_last")(x)
        ...
        for i, comb_fl in enumerate(self.combined_fc_layers, 1):
            combined = Dense(comb_fl, activation='relu',
